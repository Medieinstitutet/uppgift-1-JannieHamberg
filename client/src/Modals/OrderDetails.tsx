    ...
    <div className="relative p-5 w-full max-w-md h-auto">
        <div className="rounded shadow-lg bg-white p-5">
            ...
            <ul>
                ...
                <li><strong>Customer:</strong> {order.customer}</li><br />
                <strong>Products:</strong>
                {order.items && order.items.map((item) => (
                    <li key={item._id}>
                        {item.productDetails.name} <br /> <strong>Quantity:</strong> {item.amount}
                        <br />
                        <strong>Price per item:</strong> ${item.productDetails.price}
                        <div className='border border-dashed border-black mb-4'></div>
                    </li>
                ))}
            </ul>
